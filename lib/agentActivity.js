...
const CALL_ACTIONS = new Set(['status', 'callback']);
...
export function countAgentCalls(supplierStates, agent, sinceMs, now = Date.now()) {
  ...
  Object.values(supplierStates).forEach((state) => {
    if (Array.isArray(state.activityLog) && state.activityLog.length) {
      count += entriesInWindow(state.activityLog, agent, sinceMs, now)
        .filter((entry) => CALL_ACTIONS.has(entry.action))
        .length;
      return;
    }
    ...
    if (
      state.agent === agent &&
      state.lastTouchedAt >= now - sinceMs &&
      (state.status !== null && state.status !== undefined || state.callbackScheduled)
    ) {
      count += 1;
    }
  });
