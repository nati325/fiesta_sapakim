...
export function emptyPipelineStats() {
  return { noAnswer: 0, refused: 0, callback: 0, forwarded: 0, noStatus: 0 };
}

function isPipelineOutcomeEntry(entry) {
  return entry?.action === 'callback' || entry?.action === 'status';
}

/** Pair each outcome to the latest preceding unpaired call; leftover calls = noStatus. */
function countUnpairedCalls(agentLog) {
  const calls = agentLog
    .filter((entry) => entry.action === 'call')
    .sort((a, b) => (a.at || 0) - (b.at || 0));
  const outcomes = agentLog
    .filter(isPipelineOutcomeEntry)
    .sort((a, b) => (a.at || 0) - (b.at || 0));
  ...
  for (const outcome of outcomes) {
    ...
    for (let i = calls.length - 1; i >= 0; i -= 1) {
      ...
      if ((calls[i].at || 0) <= (outcome.at || 0)) {
        matchIdx = i;
        break;
      }
    }
    if (matchIdx >= 0) paired.add(matchIdx);
  }
  return calls.length - paired.size;
}
...
export function countAgentPipelineBetween(supplierStates, agent, fromMs, toMs) {
  ...
  Object.values(supplierStates || {}).forEach((state) => {
    ...
    stats.noStatus += countUnpairedCalls(agentLog);
  });
  return stats;
}
