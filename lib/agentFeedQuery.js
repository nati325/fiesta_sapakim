...
function filterFeedPool(suppliers, statesMap, { agent, tab, yinonWorkGroup }) {
  return suppliers
    .filter((s) => supplierBelongsToAgent(s, agent))
    .filter((s) => {
      if (agent !== 'ינון') return true;
      return supplierInYinonView(s, yinonWorkGroup || 'makeup');
    })
    .filter((s) => {
...
export async function queryTabCounts({
  ...
  for (const s of candidates) {
    if (!supplierBelongsToAgent(s, agent)) continue;
