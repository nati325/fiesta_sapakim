...
export const PHOTO_OWNER_FIELD = 'photoOwner';
export const TAL_AGENT = 'טל';
export const HODAYA_AGENT = 'הודיה';
...
export function photographerOwner(state = {}) {
  return state?.[PHOTO_OWNER_FIELD] || '';
}

export function supplierBelongsToAgent(supplier, agent, state = {}) {
  if (!agent || VIEW_ALL_AGENTS.has(agent)) return true;
  if (agent === 'ינון') return getYinonWorkGroup(supplier) != null;
  if (agent === HODAYA_AGENT) {
    return isPhotographerSupplier(supplier) && photographerOwner(state) !== TAL_AGENT;
  }
  if (agent === TAL_AGENT) {
    return isPhotographerSupplier(supplier) && photographerOwner(state) === TAL_AGENT;
  }
...
export function agentKeywordList(agent, yinonWorkGroup = '') {
  ...
  if (agent === HODAYA_AGENT || agent === TAL_AGENT) return PHOTO_KEYWORDS;
  if (agent === 'ינון') {
