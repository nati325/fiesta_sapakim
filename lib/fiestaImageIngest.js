...
/** Free GitHub CDN mirror of `public/media` (nati325/for-photos). */
...
/** Local disk first; on Vercel fall back to the free media CDN / public origin. */
async function bufferFromRelativePath(relativePath, origin = '') {
  try {
    return bufferFromLocal(relativePath);
  } catch {
    /* continue to CDN / public origin */
  }
  ...
  let lastError = new Error('לא נמצא מקור לתמונה');
  for (const base of unique) {
    try {
      return await bufferFromRemote(`${base}${relativePath}`);
    } catch (err) {
      lastError = err;
    }
  }
 ...
 */
export async function ingestImage(source, { db, label = 'image', origin = '' } = {}) {
  ...
  try {
    ...
    if (value.startsWith('data:')) {
      ...
      extracted = await bufferFromRelativePath(value, origin);
    } else {
      return value;
    }
