  ...
  return {
    ...
    types: Array.isArray(overrides.types)
      ...
      : overrides.type
        ...
        : [],
    description,
    region: normalizeFiestaRegion(
      overrides.region
      || (Array.isArray(overrides.regions) && overrides.regions[0])
      || extractRegionFromAddress(address)
    ),
    regions: normalizeFiestaRegions(
      overrides.regions,
      overrides.region || extractRegionFromAddress(address)
    ),
    ...
    eventTypes: Array.isArray(overrides.eventTypes) ? overrides.eventTypes.filter(Boolean) : [],
    eventTypesExplicit: overrides.eventTypesExplicit !== false,
    eventPrices: Array.isArray(overrides.eventPrices) ? overrides.eventPrices : [],
    fitsAllEvents: Boolean(overrides.fitsAllEvents),
  };
}
...
async function normalizeProducts(list, { origin, db } = {}) {
  ...
  return Promise.all(
    list.map(async (p, i) => ({
      ...
      eventType: String(p?.eventType || '').trim(),
...
async function buildVendorFields({ supplier, fiestaData, origin, db }) {
  ...
  // Per-event prices stay on `eventPrices`. Cloning them into products made
  // the site treat "חתונה" / "בר מצווה" as alternative packages.
  let products = await normalizeProducts(fiestaData.products, { origin, db });
  // Products without an explicit image inherit gallery / main photo (round-robin).
  const fallbackPool = [
    ...portfolio.map((item) => String(item?.image || '').trim()).filter(Boolean),
  ...
  if (fallbackPool.length) {
    ...
    products = products.map((p) => {
      ...
      const image = fallbackPool[autoIdx % fallbackPool.length];
      autoIdx += 1;
      return { ...p, image };
    });
  }
  ...
  const regions = normalizeFiestaRegions(
    fiestaData.regions,
    fiestaData.region || extractRegionFromAddress(supplier.Address || supplier.address || '')
  );
  const primaryRegion = regions[0] || normalizeFiestaRegion(fiestaData.region || '') || '';
  ...
  return {
    ...
    adminNotes: [
      ...
      `📍 אזורי אתר: ${regions.join(', ') || primaryRegion || 'לא צוין'}`,
      `🎉 אירועים: ${(Array.isArray(fiestaData.eventTypes) && fiestaData.eventTypes.length
        ? fiestaData.eventTypes
        : ['מתאים לכל האירועים']).join(', ')}`,
      ...(Array.isArray(fiestaData.eventPrices) && fiestaData.eventPrices.length
        ? fiestaData.eventPrices.map((row) =>
            `💰 ${row.eventType}: מחירון ₪${row.originalPrice || 0} · ללקוח ₪${row.price || 0} · הנחה ${row.discount || 0}% · עמלה ${row.commissionPercent || 0}% (₪${row.commissionAmount || 0})`
          )
    ...
    eventTypes: Array.isArray(fiestaData.eventTypes) && fiestaData.eventTypes.length
      ? fiestaData.eventTypes
      : ['מתאים לכל האירועים'],
    eventTypesExplicit: true,
    eventPrices: (Array.isArray(fiestaData.eventPrices) ? fiestaData.eventPrices : [])
      .map((row) => ({
...
function withoutBlankProductFields(fields) {
  ...
  if (!next.eventTypes?.length) {
    delete next.eventTypes;
    delete next.eventTypesExplicit;
  }
  // Empty eventPrices is an explicit "same price for the selected events".
  // Only skip wiping when the agent never sent event typing at all.
  if (!next.eventTypesExplicit && !next.eventPrices?.length) {
    delete next.eventPrices;
  }
  return next;
}
