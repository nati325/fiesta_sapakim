...
async function buildVendorFields({ supplier, fiestaData, origin, db }) {
  ...
  const agreementImage = await prepareAgreementImage(fiestaData.agreementImage || '', origin, db);
...
/**
 * Product data is owned by the Fiesta admin. An agent re-push that carries no
 * products must leave the existing ones alone instead of blanking them.
 */
function withoutBlankProductFields(fields) {
  if (fields.products?.length) return fields;
  const next = { ...fields };
