...
async function buildVendorFields({ supplier, fiestaData, origin, db }) {
  ...
  if (!products.length && Array.isArray(fiestaData.eventPrices) && fiestaData.eventPrices.length) {
    products = await normalizeProducts(
      fiestaData.eventPrices.map((row, i) => ({
        id: `evt${i + 1}`,
        name: row.eventType,
