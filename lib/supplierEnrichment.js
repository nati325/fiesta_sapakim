...
import { applyPreferredLocalImage } from './rankLocalMedia.js';
import { isBadImageUrl } from './supplierImageSources.js';
...
/**
 * supplier_reviews.json stores either a bare array OR a wrapper:
 * { reviews: [...], google_rating, reviews_count, last_updated }
 * Always return a plain review array for push/UI.
 */
export function normalizeReviewsList(raw) {
  if (!raw) return [];
  if (Array.isArray(raw)) {
    return raw.filter((r) => r && typeof r === 'object' && !Array.isArray(r));
  }
  if (typeof raw === 'object' && Array.isArray(raw.reviews)) {
    return raw.reviews.filter((r) => r && typeof r === 'object' && !Array.isArray(r));
  }
  return [];
}
...
function localImageExists(relativePath, cwd) {
  ...
  // On Vercel (and similar), `public/media` is served as static CDN assets and is
  // often NOT available via fs inside the serverless function. Trust the path.
...
function resolveImages(rawImages, csvRow, cwd) {
  const fromDisk = (rawImages || []).filter(
    (img) => img && (img.startsWith('http') || localImageExists(img, cwd)) && !isBadImageUrl(img)
  );
  ...
  ].filter((url) => url.startsWith('http') && !isBadImageUrl(url));
...
function mapJsonItem(item, csvRow, auxMaps, cwd) {
  ...
  return normalizeSupplierRecord({
    ...
    reviews: item.reviews?.length
      ? normalizeReviewsList(item.reviews)
      : normalizeReviewsList(auxMaps.reviews[name]),
    images,
  });
}
...
function mapCsvRow(row, auxMaps, cwd) {
  ...
  return normalizeSupplierRecord({
    ...
    description:
      auxMaps.descriptions[name]?.description ||
      (typeof auxMaps.descriptions[name] === 'string' ? auxMaps.descriptions[name] : ''),
    reviews: normalizeReviewsList(auxMaps.reviews[name]),
    images,
  });
}
...
export function normalizeSupplierRecord(supplier) {
  ...
  const images = (supplier.images || []).filter((img) => img && !isBadImageUrl(img));
  const mainImage =
    cleanValue(supplier['Main Image']) ||
    cleanValue(supplier['Google Image']) ||
    images.find((img) => img.startsWith('http') && !isBadImageUrl(img)) ||
    images[0] ||
    '';
  ...
  const normalized = {
    ...
    'Main Image': mainImage && !isBadImageUrl(mainImage) ? mainImage : '',
    'Google Image': (() => {
      const g = cleanValue(supplier['Google Image']);
      return g && !isBadImageUrl(g) ? g : '';
    })(),
    description: cleanDescription(supplier.description || ''),
    reviews: normalizeReviewsList(supplier.reviews),
    images,
  };

  // Prefer real local photos over App Store badge img_1.jpg
  try {
    return applyPreferredLocalImage(normalized);
  } catch {
    return normalized;
  }
}
