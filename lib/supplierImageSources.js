...
export function extractInstagramUrls(supplier) {
  ...
  for (const review of Array.isArray(supplier.reviews) ? supplier.reviews : []) {
...
/** Normalize /media/ vs absolute URL so fallback matching works in the browser. */
export function normalizeImageCandidate(url) {
  const value = String(url || '').trim();
  if (!value) return '';
  try {
...
/** Next candidate after rejecting a loaded badge / broken file (client-side). */
export function getNextImageCandidate(supplier, currentUrl) {
  ...
  const current = normalizeImageCandidate(currentUrl);
  const idx = unique.findIndex((u) => normalizeImageCandidate(u) === current);
  if (idx >= 0 && idx < unique.length - 1) return unique[idx + 1];
  return unique.find((u) => normalizeImageCandidate(u) !== current) || null;
}
